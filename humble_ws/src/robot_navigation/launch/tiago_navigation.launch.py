...
def generate_launch_description():
    ...
    map_dir = LaunchConfiguration(
        ...
        default=os.path.join(
            get_package_share_directory("robot_navigation"), "maps", "warehouse.yaml"
        ),
    )
    ...
    param_dir = LaunchConfiguration(
        "params_file",
        default=os.path.join(
            get_package_share_directory("robot_navigation"), "params", "tiago_navigation_params.yaml"
        ),
    )

    nav2_bringup_launch_dir = os.path.join(get_package_share_directory("nav2_bringup"), "launch")

    rviz_config_dir = os.path.join(get_package_share_directory("robot_navigation"), "rviz2", "tiago_navigation.rviz")
