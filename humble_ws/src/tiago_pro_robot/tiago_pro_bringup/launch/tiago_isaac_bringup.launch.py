...
class LaunchArguments(LaunchArgumentsBase):
    ...
    use_sim_time: DeclareLaunchArgument = DeclareLaunchArgument(
        'use_sim_time',
        default_value='True',
        description='Use simulation time (always True for Isaac Sim)'
    )
...
def declare_actions(launch_description: LaunchDescription, launch_args: LaunchArguments):
    # Start controller_manager for Isaac Sim
    from launch_ros.actions import Node
    from ament_index_python.packages import get_package_share_directory
    import os

    tiago_desc_share = get_package_share_directory('tiago_pro_description')
    controller_manager_config = os.path.join(
        tiago_desc_share, 'ros2_control', 'gazebo_controller_manager_cfg.yaml'
    )

    controller_manager_node = Node(
        package='controller_manager',
        executable='ros2_control_node',
        parameters=[controller_manager_config, {'use_sim_time': True}],
        output='both',
    ...
    # Launch default controllers (spawners for all controllers)
